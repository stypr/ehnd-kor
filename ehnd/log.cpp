...
void LogStartMsg()
{
    ...
    WriteLog(NORMAL_LOG, L"Ehnd-Kor :: 엔드 - VER. %s :: COMPILE AT %s, %s\n", WIDEN(EHND_VER), WIDEN(__DATE__), WIDEN(__TIME__));
    WriteLog(NORMAL_LOG, L"──── ━━ Ehnd -- sokcuri.neko.kr --\n");
    WriteLog(NORMAL_LOG, L"\n");
    WriteLog(NORMAL_LOG, L"- 제작자 : %s, 한국어 번역빌드: %s\n", L"소쿠릿", L"stypr");
    WriteLog(NORMAL_LOG, L"━━━━━━━━━───────────-＊\n");
...
void WriteTextLog(const wchar_t *format, ...)
{
    ...
    const char* szFileName = ".\\ehnd_kor_text.log";
...
DWORD WINAPI LogThreadMain(LPVOID lpParam)
{
    ...
    wsprintf(wszTitle, L"Ehnd-Kor Log Window :: VER. %s (%s, %s)", WIDEN(EHND_VER), WIDEN(__DATE__), WIDEN(__TIME__));
